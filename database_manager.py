...
class DatabaseManager:
    ...
    def setup_database(self):
        ...
        with psycopg2.connect(DATABASE_URL) as conn:
            with conn.cursor() as cursor:
                ...
                # roleplay_settings
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS roleplay_settings (
    ...
    def set_roleplay_settings(self, channel_id: int, settings: dict) -> bool:
        """Store roleplay settings for a channel"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                ...
                cursor.execute("""
                    SELECT 1 FROM roleplay_settings 
                    WHERE channel_id = ?
                """, (channel_id,))
                ...
                if cursor.fetchone():
                    ...
                    cursor.execute("""
                        UPDATE roleplay_settings 
                        SET settings = ? 
                        WHERE channel_id = ?
                    """, (settings_json, channel_id))
                else:
                    # Insert new settings
                    cursor.execute("""
                        INSERT INTO roleplay_settings (channel_id, settings)
                        VALUES (?, ?)
                    """, (channel_id, settings_json))

                conn.commit()
                return True
        except Exception as e:
            print(f"Error setting roleplay settings: {e}")
            return False

    def get_roleplay_settings(self, channel_id: int) -> dict:
        """Get roleplay settings for a channel"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    SELECT settings FROM roleplay_settings 
                    WHERE channel_id = ?
                """, (channel_id,))
            ...
            print(f"Error getting roleplay settings: {e}")
            return None

    def delete_roleplay_settings(self, channel_id: int) -> bool:
        """Delete roleplay settings for a channel"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    DELETE FROM roleplay_settings 
                    WHERE channel_id = ?
                """, (channel_id,))
                conn.commit()
                return True
        except Exception as e:
            print(f"Error deleting roleplay settings: {e}")
            return False
    ...
    def create_tables(self):
        ...
        try:
            with self.get_connection() as conn:
                ...
                # Create roleplay_settings table
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS roleplay_settings (
                        channel_id INTEGER PRIMARY KEY,
                        settings TEXT NOT NULL,
    ...
    def create_memory_tables(self):
        with self.conn.cursor() as cur:
            ...
            CREATE INDEX IF NOT EXISTS idx_episodes_user_char_time ON episodes(user_id, character, timestamp);
    ...
    def get_profile(self, user_id, character):
        with self.conn.cursor() as cur:
            cur.execute('''
                SELECT key, value FROM profiles WHERE user_id=%s AND character=%s
            ''', (user_id, character))
    ...
    def add_episode(self, user_id, character, summary, timestamp):
        with self.conn.cursor() as cur:
            cur.execute('''
                ...
                VALUES (%s, %s, %s, %s)
            ''', (user_id, character, summary, timestamp))
            self.conn.commit()
    ...
    def delete_old_episodes(self, days=20):
        with self.conn.cursor() as cur:
            cur.execute('''
                DELETE FROM episodes WHERE timestamp < NOW() - INTERVAL '%s days'
            ''', (days,))
            self.conn.commit()
    ...
    def get_states(self, user_id, character):
        with self.conn.cursor() as cur:
            cur.execute('''
                SELECT emotion_type, score FROM states WHERE user_id=%s AND character=%s
            ''', (user_id, character))
            return dict(cur.fetchall()) 